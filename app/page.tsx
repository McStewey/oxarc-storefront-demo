...
export default function Home() {
  ...
  const discount = tier === 'Business' ? .92 : tier === 'Volume' ? .84 : 1;
  ...
  return <main>
    ...
    <section className="category-strip"><div><span>⚡</span><b>Welding & Cutting</b><small>Machines, torches & consumables</small></div><div><span>◉</span><b>Industrial Gases</b><small>Cylinders, bulk & specialty</small></div><div><span>⛨</span><b>Safety & PPE</b><small>Head-to-toe protection</small></div><div><span>◆</span><b>Tools & Abrasives</b><small>Built for demanding work</small></div></section>
    <section className="shop" id="shop"><div className="section-head"><div><span className="kicker">SEARCHABLE INDUSTRIAL CATALOG</span><h2>Popular products</h2></div><div className="tier"><span>Your pricing:</span>{['Public','Business','Volume'].map(t=><button key={t} onClick={()=>setTier(t)} className={tier===t?'active':''}>{t}</button>)}</div></div>
      ...
      <div className="catalog"><aside className={showFilters?'open':''}><div><b>Product category</b>{['All',...new Set(products.map(p=>p.category))].map(x=><label key={x}><input type="radio" name="cat" checked={category===x} onChange={()=>setCategory(x)}/>{x}<small>{x==='All'?products.length:products.filter(p=>p.category===x).length}</small></label>)}</div><div><b>Brand</b><select value={brand} onChange={e=>setBrand(e.target.value)}><option>All</option>{[...new Set(products.map(p=>p.brand))].map(x=><option key={x}>{x}</option>)}</select></div><div><b>Availability</b><label><input type="checkbox" checked={stockOnly} onChange={e=>setStockOnly(e.target.checked)}/> In stock now</label></div><button className="clear" onClick={()=>{setCategory('All');setBrand('All');setStockOnly(false);setQuery('')}}>Clear all filters</button></aside>
        <div className="products">{visible.map(p=><article key={p.id}><div className="product-image"><span>{p.badge}</span><img src={p.image} alt=""/><button aria-label="Save product">♡</button></div><div className="product-body"><small>{p.brand.toUpperCase()} · MODEL {p.sku}</small><h3>{p.name}</h3><div className="rating">★★★★★ <span>4.8</span></div><div className="price"><b>${(p.price*discount).toFixed(2)}</b><small>/ each</small></div><div className="delivery"><i/><span><b>{p.delivery}</b><small>{p.availability} · Nearest OXARC branch</small></span></div><button className="add" onClick={()=>setCart(c=>c+1)}>Add to cart</button></div></article>)}{!visible.length&&<div className="no-results">No exact matches. Try a product type, brand, model, or part number.</div>}</div>
      </div>
    </section>
  </main>;
}
